...
def two_sum_2?(array, target) #O(nlogn)
  array = array.sort

  sum_binary_search(array, target)
end

def sum_binary_search(arr, target)
  mid = arr.length / 2
  sum = arr[mid] + arr[mid - 1]
  # sum_more = arr[mid] + arr[mid + 1]
  ...
  if sum < target
    sum_binary_search(arr[mid..-1], target)
  else #sum > target
    sum_binary_search(arr[0..mid], target)
  end

end

 arr = [0, 2, 5, 7]
 puts two_sum_2?(arr, 6)

 #[1,8,3,6,9,5,4] ==> 9

def two_sum_hash?(arr, target)
  hash = Hash.new(0)
  arr.each { |el| hash[el] = target - el } #O(n)

  arr.each do |el| #O(n)
    return true if hash.has_key(hash[el]) #O(1)
  end
  false
end
